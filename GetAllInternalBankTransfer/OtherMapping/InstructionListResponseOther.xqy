        ...
        <ns3:data>
          ...
          <ns3:errorList>
            ...
            <ns3:startSequence></ns3:startSequence>
                 {
  for $tx in $Response/ns1:operationData/ns1:CTFO_O_0003
  where (
  fn:normalize-space($refNo) = '' 
  or fn:data($tx/ns1:CTFO_O_0003_0001) = $refNo
)
  return
            <ns3:instructions>
                <ns3:dictionaryArray></ns3:dictionaryArray>
                <ns3:partyId>{ $userIdVariable }</ns3:partyId>
                <ns3:externalReferenceNo>{fn:data($tx/ns1:CTFO_O_0003_0001)}</ns3:externalReferenceNo>
                <ns3:transactionType>INTERNALFT</ns3:transactionType>
<ns3:startDate>{
  concat(
    substring($tx/ns1:CTFO_O_0003_0010, 1, 4), "-", 
    substring($tx/ns1:CTFO_O_0003_0010, 5, 2), "-", 
    substring($tx/ns1:CTFO_O_0003_0010, 7, 2), 
    "T00:00:00"
  )
}</ns3:startDate>
...
<ns3:endDate>{
  concat(
    substring($tx/ns1:CTFO_O_0003_0011, 1, 4), "-", 
    substring($tx/ns1:CTFO_O_0003_0011, 5, 2), "-", 
    substring($tx/ns1:CTFO_O_0003_0011, 7, 2), 
    "T00:00:00"
  )
}</ns3:endDate>
                    <ns3:nextExecutionDate></ns3:nextExecutionDate>

                <ns3:instructionAmount>
                    <ns3:currency>{fn:data($tx/ns1:CTFO_O_0003_0004)}</ns3:currency>
<ns3:amount>
 {
              let $amount := xs:decimal(fn:data($tx/ns1:CTFO_O_0003_0003)) div 100
              return
                ...
                <ns3:debitAccountId>{fn:data($tx/ns1:CTFO_O_0003_0002)}</ns3:debitAccountId>
                <ns3:branchId>{fn:data($CDODResponse/ns2:operationData/ns2:CDOD_O_0003)}</ns3:branchId>
                <ns3:creditAccountId>{fn:data($tx/ns1:CTFO_O_0003_0005)}</ns3:creditAccountId>
                <ns3:creditAccountBranchId>{fn:data($CDODResponse/ns2:operationData/ns2:CDOD_O_0003)}</ns3:creditAccountBranchId>
                <ns3:beneficiaryName>{fn:data($tx/ns1:CTFO_O_0003_0006)}</ns3:beneficiaryName>
                <ns3:remarks>{fn:data($tx/ns1:CTFO_O_0003_0009)}</ns3:remarks>
 <ns3:status>{
  let $status1 := normalize-space($tx/ns1:CTFO_O_0003_0013)
  return
    if ($status1 = 'Aguarda débito') then 'O'
    else if ($status1 = 'Comunicada') then 'O'
    else if ($status1 = 'Aguarda envio') then 'O'
    else if ($status1 = 'Pendente aprovação') then 'O'
    else if ($status1 = 'Envio suprimido') then 'C'
    else if ($status1 = 'Anulada/Cancelada') then 'C'
    else ' '
}</ns3:status>
                <ns3:purposeId>NULL</ns3:purposeId>
             {
let $freq := fn:data($tx/ns1:CTFO_O_0003_0012)
let $endDate := fn:data($tx/ns1:CTFO_O_0003_0010)
 ...
 <ns3:status>{
  ...
  return
    if ($status1 = 'Aguarda débito') then 'O'
                    ...
                    <ns3:failureReason>{fn:data($tx/ns1:CTFO_O_0003_0013)}</ns3:failureReason>
                </ns3:paymentDetailsList>
            </ns3:instructions>
            }
        </ns3:data>
