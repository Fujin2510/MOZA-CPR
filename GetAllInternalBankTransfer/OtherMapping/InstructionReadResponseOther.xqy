...
declare function local:func($Response as element() (:: schema-element(ns1:Response) ::),$userIdVariable,$value,$refNo,$status) as element() (:: schema-element(ns2:Response) ::) {
  ...
  where fn:data($tx/ns1:CTFO_O_0003_0001) = $refNo
  return    
            <ns2:instruction>
                <ns2:dictionaryArray></ns2:dictionaryArray>
                <ns2:partyId>{ $userIdVariable }</ns2:partyId>               
                <ns2:externalReferenceNo>{fn:data($tx/ns1:CTFO_O_0003_0001)}</ns2:externalReferenceNo>
...
<ns2:endDate>{
  concat(
    ...
    "T00:00:00"
                    ...
                    <ns2:nextExecutionDate></ns2:nextExecutionDate>

                <ns2:instructionAmount>
                    <ns2:currency>{fn:data($tx/ns1:CTFO_O_0003_0004)}</ns2:currency>
                    <ns2:amount> {
              ...
              return
                ...
                <ns2:debitAccountId>{fn:data($tx/ns1:CTFO_O_0003_0002)}</ns2:debitAccountId>
                <ns2:branchId>{$value}</ns2:branchId>
                <ns2:creditAccountId>{fn:data($tx/ns1:CTFO_O_0003_0005)}</ns2:creditAccountId>
                <ns2:creditAccountBranchId>{$value}</ns2:creditAccountBranchId>
                <ns2:beneficiaryName>{fn:data($tx/ns1:CTFO_O_0003_0006)}</ns2:beneficiaryName>
                <ns2:remarks>{fn:data($tx/ns1:CTFO_O_0003_0009)}</ns2:remarks>
 <ns2:status>{
