...
declare namespace ns2="http://www.mozabank.org/ccdo_casaaccount_partylist_summary";
...
declare function local:func($CDOD as element() (:: schema-element(ns1:Response) ::)) as element() (:: schema-element(ns2:Response) ::) {
    ...
    <ns2:data>
        ...
        <ns2:result>
          ...
          <!--  <ns2:status>{if(fn:data($CDOD/ns1:errorCode) = 0) then 'SUCCESS' else 'FAILURE'}</ns2:status>
            {if(fn:data($CDOD/ns1:errorCode) = 0) then () else(
			<ns2:errorList>
				<ns2:code>{ dvm:lookup('CommonErrorHandlerService/ErrorCodes.dvm', 'MSBCode',xs:string(fn:data($CDOD/ns1:errorCode)), 'ErrorCode',"ERR001") }</ns2:code>
				<ns2:message>{ dvm:lookup('CommonErrorHandlerService/ErrorCodes.dvm', 'MSBCode',xs:string(fn:data($CDOD/ns1:errorCode)), 'ErrorMessage',"Invalid backend response") }</ns2:message>
			</ns2:errorList>)
			}-->
        ...
        <ns2:startSequence></ns2:startSequence>
         ...
         {
                 if( $errCode = '0') then
         for $cdod in $CDOD/ns1:operationData/ns1:CCDO_O_0003
         return  
        <ns2:accounts>
            <ns2:dictionaryArray></ns2:dictionaryArray>
            ...
            <ns2:accountDisplayName>{fn:data($CDOD/ns1:user)}</ns2:accountDisplayName>
            <ns2:currency>{fn:data($cdod/ns1:CDOD/ns1:CDOD_O_0015)}</ns2:currency>
            <ns2:status>ACTIVE</ns2:status>
            <ns2:balance>
                <ns2:currency>{fn:data($cdod/ns1:CDOD/ns1:CDOD_O_0015)}</ns2:currency>
                <ns2:amount>{fn:data($cdod/ns1:CDOD/ns1:CDOD_O_0007)}</ns2:amount>
            </ns2:balance>
            ...
            <ns2:sortCode>{fn:data($cdod/ns1:CDOD/ns1:CDOD_O_0003)}</ns2:sortCode>
            <ns2:relation></ns2:relation>
            <ns2:moduleType></ns2:moduleType>
            <ns2:iban>{fn:data($cdod/ns1:CDOD/ns1:CDOD_O_0026)}</ns2:iban>
        </ns2:accounts> 
        else()
        }
        {
                 if( $errCode = '0') then
        <ns2:count>{count($CDOD/ns1:operationData/ns1:CCDO_O_0003)}</ns2:count>    else()
        }
        </ns2:data>
    </ns2:Response>
};
