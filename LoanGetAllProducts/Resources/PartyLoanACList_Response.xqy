...
declare namespace ns2="http://www.mozabank.org/PARTY_LOAN_ACCOUNTS_LIST";
(:: import schema at "Schema/PARTY_LOAN_ACCOUNTS_LIST.xsd" ::)

declare namespace dvm ="http://www.oracle.com/osb/xpath-functions/dvm";
declare variable $CCRD_Response as element() (:: schema-element(ns1:CCCRResponse) ::) external;

declare function local:func($CCRD_Response as element() (:: schema-element(ns1:CCCRResponse) ::)) as element() (:: schema-element(ns2:PARTY_LOAN_ACCOUNTS_LISTResponse) ::) {
   
     let $errCode := fn:data($CCRD_Response/*:errorCode) return
    <ns2:PARTY_LOAN_ACCOUNTS_LISTResponse>
     <ns2:data>
          <ns2:result>
         <!--   <ns2:status>
            {
            ...
            {if (fn:data($CCRD_Response/ns1:errorCode) = '0') then
  ()
else
  <ns2:errorList>
    <ns2:code>{fn:data($CCRD_Response/ns1:errorCode)}</ns2:code>
      <ns2:message>{fn:data($CCRD_Response/ns1:errorMessage/ns1:messages)}</ns2:message>
  </ns2:errorList>}-->
     <ns2:status>{  if( $errCode = '0' or $errCode = 'P' or $errCode ='B') then 'SUCCESS' else 'FAILURE'}</ns2:status>
        ...
        <ns2:accounts>
            ...
            <ns2:accountDisplayName>{fn:data($ccrd/ns1:CCCR_O_0003_0002)}</ns2:accountDisplayName>
            <ns2:currency>{fn:data($ccrd/ns1:CCCR_O_0003_0006)}</ns2:currency>
                ...
                <ns2:amount>
                      {
                        fn-bea:format-number(
                          xs:decimal(fn:data($ccrd/ns1:CCCR_O_0003_0005)) div 100,
                          '0.00'
                        )
                      }
                </ns2:amount>
            ...
            <ns2:openingDate>
            {let $date := fn:data($ccrd/ns1:CCRD/ns1:CCRD_O_0005) return 
            ...
            <ns2:sortCode>{fn:data($ccrd/ns1:CCRD/ns1:CCRD_O_0003)}</ns2:sortCode>
