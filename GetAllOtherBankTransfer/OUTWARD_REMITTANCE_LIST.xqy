...
declare namespace ns1="http://www.mozabanca.org/obdx/OUTWARD_REMITTANCE_LIST";
(:: import schema at "OUTWARD_REMITTANCE_LIST.xsd" ::)

declare variable $OUTWARD_LIST as element() (:: schema-element(ns1:Request) ::) external;
declare variable $userIdVar as xs:string external;

declare function local:func($OUTWARD_LIST as element() (:: schema-element(ns1:Request) ::),$UserIdVar as xs:string) as element() (:: schema-element(ns2:Request) ::) {
    <ns2:Request>
        ...
        <ns2:operationData>
            <ns2:CTFO_I_0001>{
    let $acct := fn:data($OUTWARD_LIST/ns1:accountList)
    return
      if (contains($acct, "@~")) then
        ...
        $acct
            ...
            <ns2:CTFO_I_0007>{fn:data($OUTWARD_LIST/ns1:fromDate)}</ns2:CTFO_I_0007>
            <ns2:CTFO_I_0008>{fn:data($OUTWARD_LIST/ns1:toDate)}</ns2:CTFO_I_0008>
...
local:func($OUTWARD_LIST,$userIdVar)