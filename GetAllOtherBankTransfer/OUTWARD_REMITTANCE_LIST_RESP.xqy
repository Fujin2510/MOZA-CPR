...
declare variable $OUTWARD_REMITTANCE_LIST as element() (:: schema-element(ns1:Response) ::) external;

declare function local:func($UserIdVar as xs:string , 
                            $OUTWARD_REMITTANCE_LIST as element() (:: schema-element(ns1:Response) ::)) 
                            as element() (:: schema-element(ns2:Response) ::) {
   <ns2:Response>
    <ns2:data>
    <ns2:result>
        <ns2:status>  {
                    if (fn:data($OUTWARD_REMITTANCE_LIST/ns1:errorCode) = '0')
                    then 'SUCCESS'
                    else 'FAILURE'
                  }</ns2:status>              
    ...
    {
            for $CTFO in $OUTWARD_REMITTANCE_LIST/ns1:operationData/ns1:CTFO_O_0003
            return
        <ns2:outwardremittances>
                ...
                <ns2:orderingPartyId>{fn:data($UserIdVar)}</ns2:orderingPartyId>
                <ns2:debitAmount>
                ...
                <ns2:partyId>{fn:data($UserIdVar)}</ns2:partyId>
...
local:func($UserIdVar, $OUTWARD_REMITTANCE_LIST)